...
export default function SystemSummary({ userStats, bookingStats }: SystemSummaryProps) {
  ...
  const roleData = [
    { name: "Usuarios", value: userStats.usersByRole.user, color: "var(--chart-1)" },
    { name: "Admins", value: userStats.usersByRole.admin, color: "var(--chart-2)" },
    { name: "IT", value: userStats.usersByRole.it_admin, color: "var(--chart-3)" },
  ];

  // Prepare data for Booking status pie chart
  const bookingStatusData = [
    { name: "Vigentes", value: Math.max(bookingStats.totalBookings - bookingStats.totalCancelled, 0), color: "var(--chart-1)" },
    { name: "Canceladas", value: bookingStats.totalCancelled, color: "var(--destructive)" },
  ];
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 sm:gap-4">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between p-2 sm:p-4 pb-1 sm:pb-2">
            <CardTitle className="text-xs sm:text-sm font-medium text-muted-foreground">Total Usuarios</CardTitle>
          ...
          <CardContent className="p-2 sm:p-4 pt-0 sm:pt-0">
            ...
            <p className="text-[10px] sm:text-xs text-muted-foreground mt-0.5 sm:mt-1 flex items-center">
              <UserPlus className="h-2 w-2 sm:h-3 sm:w-3 mr-0.5 sm:mr-1 text-success" />
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between p-2 sm:p-4 pb-1 sm:pb-2">
            <CardTitle className="text-xs sm:text-sm font-medium text-muted-foreground">Total Reservas</CardTitle>
          ...
          <CardContent className="p-2 sm:p-4 pt-0 sm:pt-0">
            ...
            <p className="text-[10px] sm:text-xs text-muted-foreground mt-0.5 sm:mt-1 flex items-center">
              {bookingGrowth >= 0 ? (
                <ArrowUpRight className="h-2 w-2 sm:h-3 sm:w-3 mr-0.5 sm:mr-1 text-success" />
              ) : (
                <ArrowDownRight className="h-2 w-2 sm:h-3 sm:w-3 mr-0.5 sm:mr-1 text-destructive" />
              )}
        ...
        <Card>
          ...
          <CardContent className="p-2 sm:p-4 pt-0 sm:pt-0">
            ...
            <p className="text-[10px] sm:text-xs text-muted-foreground mt-0.5 sm:mt-1 flex items-center">
              <AlertCircle className="h-2 w-2 sm:h-3 sm:w-3 mr-0.5 sm:mr-1 text-destructive" />
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between p-2 sm:p-4 pb-1 sm:pb-2">
            <CardTitle className="text-xs sm:text-sm font-medium text-muted-foreground">Asistentes</CardTitle>
          ...
          <CardContent className="p-2 sm:p-4 pt-0 sm:pt-0">
            ...
            <p className="text-[10px] sm:text-xs text-muted-foreground mt-0.5 sm:mt-1 flex items-center">
              <Clock className="h-2 w-2 sm:h-3 sm:w-3 mr-0.5 sm:mr-1 text-warning-foreground" />
              Media por reserva
            </p>
          </CardContent>
        </Card>
          ...
          <div className="h-64 sm:h-80 pt-2">
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <Bar dataKey="count" name="Reservas" fill="var(--chart-1)" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </CardContent>
